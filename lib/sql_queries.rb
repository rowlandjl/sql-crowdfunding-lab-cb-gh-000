...
"select title, (sum(amount) - funding_goal) as over
from projects p
left outer join pledges p2 on p.id = p2.project_id
group by title
having over >= 0"
end
...
"select category, sum(amount) from projects p left join pledges p2 on p.id = p2.project_id where category = 'books'"
end
